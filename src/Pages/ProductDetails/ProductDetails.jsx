...
const ProductDetails = () => {
    ...
    return (
        <div className="px-[5%] md:px-[10%] mx-auto p-6 mt-25 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 rounded-xl shadow p-4 border border-gray-200">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {product.tags?.map((tag, i) => (
                            <span key={i} className="text-xs bg-blue-100 text-black px-2 py-1 rounded">{tag}</span>
                        ))}
                    </div>
                    <div className="flex gap-5 items-center">
                        <div>
                            <a href={product.externalLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 underline mb-4">
            ...
            <div>
                ...
                <div className="grid md:grid-cols-2 gap-4 rounded-lg">
                    {reviews.length != 0 ? (reviews.map((review, i) => (
                        <div key={i} className="p-4 rounded-xl shadow border border-gray-200">
                    ...
                    ))) : (<h1 className="">No one reviewed yet</h1>)
                    }
                </div>
            </div>
            {/* === Post Review Section (above the reviews) === */}
            {user && (
                <form onSubmit={handleReviewSubmit} className="p-4 rounded-xl shadow mb-6">
                    ...
                    <button type="submit" className="btn bg-blue-800 text-white mt-4">
                        Submit Review
                    </button>
                </form>
            )}
